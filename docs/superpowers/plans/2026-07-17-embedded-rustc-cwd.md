...
`CwdAwareFileSystem` also carries a rubrc-owned workaround for the pinned WVL dynamic filesystem's append-only `fd_write_raw`. Although `fd_seek_raw` updates the descriptor cursor, WVL ignores it when writing. This breaks rustc's rlib/rmeta archive metadata backpatching and leaves corrupt metadata that the next rustc reports as E0786. The pinned `path_open_raw` also ignores requested descriptor flags, so the wrapper must retain those flags after a successful open. It must delegate fd 0/1/2 unchanged and use the LFS `fd_pwrite_raw` guest-memory path at the current cursor for normal dynamic descriptors. For `FDFLAGS_APPEND`, it must acquire a global append-write mutex after the descriptor entry and hold it across current inode-size lookup, all positioned iovec writes, descriptor cursor update, and `nwritten` storage. This descriptor-entry-then-append-mutex order serializes APPEND across descriptors; no path may acquire those locks in reverse, and non-append writes must not take the append mutex. Remove these overrides once the pinned WVL preserves open flags and makes `StandardDynamicFileSystem::fd_write_raw` cursor-aware.

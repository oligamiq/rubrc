...
const memoryReserveCount = parsePositiveInt(
  Deno.env.get("VFS_DEBUG_MEMORY_RESERVE_COUNT"),
  1,
  "VFS_DEBUG_MEMORY_RESERVE_COUNT",
);
const memoryReservePages = parsePositiveInt(
  Deno.env.get("VFS_DEBUG_MEMORY_RESERVE_PAGES"),
  1024,
  "VFS_DEBUG_MEMORY_RESERVE_PAGES",
);
const rustcMemoryReservePages = parsePositiveInt(
  Deno.env.get("VFS_DEBUG_RUSTC_MEMORY_RESERVE_PAGES"),
  4096,
...
const worker = new Worker(
  new URL("./debug_rustc_shell_thread_spawn_worker.ts", import.meta.url),
  { type: "module" },
);

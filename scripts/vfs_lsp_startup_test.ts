Deno.test({
  ...
  async fn() {
    ...
    for (const [name, src] of [["lib.rs", libSrc], ["command.rs", cmdSrc]]) {
      ...
      const initPattern = /lsp_opt::_reset\(\);\s*(?:crate::)?lsp_opt::_start\(\);\s*(?:crate::)?lsp_opt::_main\(\);/;
      if (!initPattern.test(src)) {
        throw new Error(`${name} does not call lsp_opt::_reset(), lsp_opt::_start(), and lsp_opt::_main() in exact sequential order`);
      }
    }
  }
});
